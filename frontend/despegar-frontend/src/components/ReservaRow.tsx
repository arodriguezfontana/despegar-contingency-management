...
export const ReservaRow: React.FC<Props> = ({ reserva, onGestionar, isLoading }) => {
  ...
  return (
    <tr className={`border-b border-gray-100 transition-colors ${isResuelto ? 'bg-gray-50/50' : 'hover:bg-[#f5f7fa]'}`}>
        ...
        <div className={`flex flex-col gap-3 ${isResuelto ? 'opacity-50' : ''}`}>
          ...
          {reserva.nombreHotel && (
            <div className="pt-2 border-t border-gray-50">
              ...
              <div
                className={`text-[11px] font-bold mt-0.5 ${reserva.estadoHotel === 'NO_DISPONIBLE' ? 'text-[#fa503f]' : 'text-gray-400'
                  }`}
              >
                {(reserva.estadoHotel || 'Estado desconocido').replace('_', ' ')}
              </div>
            </div>
      ...
      <td className="p-5">
        {!isResuelto && (
          <span className={`text-[10px] font-bold px-4 py-1.5 rounded-full border-none shadow-sm tracking-tighter uppercase animate-in fade-in duration-500 ${isCritical
